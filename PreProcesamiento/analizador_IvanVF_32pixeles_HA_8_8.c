...
Se parte del código desarrollado por el autor, Ignacio Sotomonte y Victor Morales Gomez.
 ...
 https://www.tutorialspoint.com/data_structures_algorithms/hash_table_program_in_c.htm
  ...
  Este código solo es válido para las clases FileTransfer, Video y Audio.
...
*/			
	...
	// variables de la curva de Hilbert
	char curva[DIM][DIM];
    int tam_matriz = 32;  // Tamaño de la matriz, en este caso 32x32
    int x, y;

	memset(curva, '\0', sizeof(curva));

	

	// // Calcula el tamaño total del array en bytes
    // size_t tamano_bytes = sizeof(pack);

    // // Calcula el tamaño de un solo elemento (en este caso, uint8_t)
    // size_t tamano_elemento = sizeof(uint8_t);

    // // Calcula el número de elementos en el array dividiendo el tamaño total por el tamaño de un elemento
    // size_t num_elementos = tamano_bytes / tamano_elemento;

	// printf("Tamaño total del array en bytes: %zu\n", tamano_bytes);

	// printf("Número de elementos en el array: %zu\n", num_elementos);
	// // Convierto size_t a int mediante un casting explícito
    // int num_elementos_entero = (int)num_elementos;
	// printf("Tamaño convertido a int: %d\n", num_elementos_entero);

	// // Calcula la longitud de la cadena utilizando strlen
    // size_t longitud = strlen(pack);
    
    // // Muestra la longitud de la cadena
    // printf("La longitud de la cadena es: %zu\n", longitud);

	// size_t num_elementos2 = sizeof(pack) / sizeof(*pack);

	// // Muestra la longitud de la cadena
    // printf("La longitud del pack: %zu\n", num_elementos2);

    // // Recorro el array e imprimo cada valor
    // for (size_t i = 0; i < 2024; i++) {
    //     printf("Valor en el índice %zu: %u\n", i, pack[i]);
    // }
	int cont = 0;
	...
	int l = 0;
	char *binario_inicial = (char *)malloc(cont * sizeof(char));

	if (binario_inicial == NULL) {
	...
	for (i = 0; i < 1024; i++){
		...
		d2xy(tam_matriz, i, &x, &y);
		// printf("%c", binario_inicial[k]);
		curva[x][y] = binario_inicial[k];
		// printf("\n%d--%d\n", cont, k);
		k++;
