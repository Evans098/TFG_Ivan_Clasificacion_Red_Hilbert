...
*/			
    ...
    for(int k=0; k<5;k++){
		...
		if(k==0){
			...
			char *binario_inicial_aux = (char *)malloc(cont * sizeof(char));

			if (binario_inicial_aux == NULL) {
			...
			for (i = 0; i < 1024; i++){
				...
				// printf("%c", binario_inicial[m]);
				binario_inicial[i]=binario_inicial_aux[m];
				curva[x][y] = binario_inicial_aux[m];
				// printf("\n%d--%d\n", cont, k);
				m++;
			...
			for (int l=0; l< SIGNAL_LENGTH; l++){
				d2xy(SIGNAL_LENGTH, l, &x, &y);
				curva[x][y] = senial_flip[l];
			}
			...
			for (int l=0; l< SIGNAL_LENGTH; l++){
				d2xy(SIGNAL_LENGTH, l, &x, &y);
				curva[x][y] = senial_traslada[l];
			}
		}
		// else if(k==5){
		// 	// Agregar ruido gaussiano a la característica Tamaño con alpha = 0.5 y sigma = 1.0
    	// 	float alpha = 0.5;
    	// 	float sigma = 1.0;
		// 	char senial_gaussian[SIGNAL_LENGTH] = {0};
    	...
    	// 	// }
    	// 	// printf("\n\n");
		// 	memcpy(senial_gaussian, binario_inicial, sizeof(binario_inicial));

    	// 	add_gaussian_noise(senial_gaussian, 1, alpha, sigma);

		// 	// printf("Señal gaussina:\n");
    	// 	// for (int i = 0; i < SIGNAL_LENGTH; i++) {
    	// 	//     printf("%c ", senial_gaussian[i]);
    	// 	// }
    	// 	// printf("\n\n");
		// }
